...
ShapeFile - PHP Class to read an ESRI Shapefile and its associated DBF
	...
	Edited by		: Sulistyo Sudarmono (add reads PointZ,MultiPointZ,PolylineZ,PolygonZ,PointM,MultiPointM,PolylineM,PolygonM,CheckNextRecord)
	2nd Patch       : add skip to next record if the file is corrupt
****************************************************************************************************/
...
class ShapeFile {
    ...
    public function getRecord($geometry_format = self::GEOMETRY_ARRAY)
    {
		...
		//force read if error
		while ((ftell($this->shp_handle) < $this->shp_file_size) && $this->CheckNextRecord()!==$this->shape_type) $this->ReadData('V');
    ...
    private function CheckNextRecord()
    {
		if (ftell($this->shp_handle) >= $this->shp_file_size) return false;	
		$pos = ftell($this->shp_handle);
        fread($this->shp_handle, self::$binary_data_lengths['N']);
		fread($this->shp_handle, self::$binary_data_lengths['N']);
		$data = fread($this->shp_handle, self::$binary_data_lengths['V']);
		fseek($this->shp_handle,$pos);
        if (!$data) return false;
        return current(unpack('V', $data));
    }
	...
	private function ReadZProperty()
    {
        return array(
            'zmin'  => $this->ReadData('d'),
            'zmax'  => $this->ReadData('d')
        );
    }	
	
	private function ReadMProperty()
    {
        return array(
            'mmin'  => $this->ReadData('d'),
            'mmax'  => $this->ReadData('d')
        );
    }	
    ...
    private function ReadPointZ()
    {
        $ret =  array(
       	...
       	if ($this->CheckNextRecord() !== $this->shape_type) {
			$ret['m'] = $this->ReadData('d');
		}			
    ...
    private function ReadPolyLineM()
    {
		...
		$shape_type = $this->CheckNextRecord();
		$ret['next_type'] = $shape_type." ".$this->shape_type;
       	if ($shape_type !== $this->shape_type) {
			// M Property
			$this->ReadMProperty();
			for ($i=0; $i<$tot_points; $i++) {
				$this->ReadData('d');
