...
class UploadClass{
    ...
    function ValidateExistance()
    {
        ...
        if ($this->Replace) {
            $this->SetMessage("Message: The file name '$FileName' already exist (replace mode = on).");
            return false;    
        } else if (file_exists($File)) {
            $this->SetMessage("Message: The file '$FileName' already exist.");
            $UniqueName = rand() . $FileName; // beri nama baru
...
/*
		...
		*/
            ...
            if (is_uploaded_file($TempFileName)) { 
                if (move_uploaded_file($TempFileName, $UploadDirectory . $FileName)) {
                    ...
                    $this->SetMessage("ERROR: fileupload function return FALSE");
                    die($this->GetMessage());
                    return false;
                }
            } else {
    ...
    function ReplaceFile($argv)
    {
        return $this->Replace = $argv;
    }
}
